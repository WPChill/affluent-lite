...
class Affluent_Customize_Section_Recommend extends WP_Customize_Section {
	...
	public function json() {
		...
		$json['facebook_text'] = __( 'Facebook', 'affluent' );
		$json['twitter'] = $this->twitter;
		$json['twitter_text'] = __( 'Twitter', 'affluent' );
		$json['wp_review'] = $this->wp_review;
		$json['wp_review_text'] = __( 'Review this theme on w.org', 'affluent' );
		if ( $this->wp_review ) {
			...
			<div class="recomended-actions_container" id="plugin-filter">
						...
						<div class="epsilon-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
							<# if( !data.recommended_plugins[action].check ){ #>
								<div class="epsilon-recommeded-actions">
									...
									<span data-action="dismiss" class="dashicons dashicons-visibility affluent-recommended-plugin-button" id="{{ data.recommended_plugins[action].id }}"></span>
				...
				<# if( data.required_actions.length == 0 && data.recommended_plugins.length == 0 ){ #>
					<p class="succes">
						...
						<# } #>
						<# if( data.wp_review ){ #>
							<a href="https://wordpress.org/support/theme/{{ data.theme_slug }}/reviews/#new-post" class="button button-primary affluent-wordpress" target="_blank"><span class="dashicons dashicons-wordpress"></span>{{ data.wp_review_text }}</a>
						<# } #>
					</p>
				<# }else{ #>
					<p class="succes hide">
